...
class EventSearchComponent extends Component
{
    ...
    public function render()
    {
        ...
        if($this->venue != 'all' && $this->city != 'all' )
        {
            ...
            $searchVenue = Event::where('venue', $this->venue)->where('status', '1')->where('admstatus', '1')->orderBy('startdate','ASC')->get();
        }
        elseif($this->venue == 'all' && $this->city != 'all')
        {
            //dd('tst2',$this->venue, $this->country, $this->city);
           $searchVenue = Event::where('city', $this->city)->where('status', '1')->where('admstatus', '1')->orderBy('startdate','ASC')->get();
        }
        elseif($this->venue == 'all' && $this->city == 'all')
        {
